...
export default function Map({ shipmentDetails }) {
  ...
  const NearestRoadsToSanp = nearestRoads
    ?.map((coord) => `${coord.location.latitude},${coord.location.longitude}`)
    ?.join("|");
    ...
    <>
      <Grid container mt={2}>
        <Typography variant="h6">
          Snap To Road Use With Nearest Roads Marker{" "}
        </Typography>
