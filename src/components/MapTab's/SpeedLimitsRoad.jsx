...
export default function Map({ shipmentDetails }) {
  ...
  const [speedLimitRoads, setSpeedLimitRoads] = useState([]);
  ...
  const originDestination = [originCoord + destinationCoordinates];
  ...
  useEffect(() => {
    const fetchDataSpeedLimitRoads = async () => {
      try {
        ...
        setSpeedLimitRoads(NearestPoints);
      } catch (error) {
        setError(error);
      }
    };
    if (flatArrayLimit) {
      fetchDataSpeedLimitRoads();
    }
  }, []);
  ...
  return (
    <>
      <Grid container mt={2}>
        <Typography variant="h6">Speed Limit </Typography>
        <LoadScript googleMapsApiKey={MAP_KEY} ref={mapRef}>
          ...
          >
            ...
            {speedLimitRoads.map((point, index) => (
              <Marker
                position={{
            ...
            <Polyline
              path={speedLimitRoads?.map((point) => ({
                lat: point.location.latitude,
                lng: point.location.longitude,
              }))}
              options={{
